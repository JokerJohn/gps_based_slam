...
void vio_callback(const nav_msgs::Odometry::ConstPtr &pose_msg) {
  ...
  while (!gpsQueue.empty()) {
    ...
    if (gps_t >= t - 0.05 && gps_t <= t + 0.05) {
      ...
      double pos_accuracy2 = GPS_msg->position_covariance[4];
      double pos_accuracy3 = GPS_msg->position_covariance[8];
      std::cout << "gps_error: " << pos_accuracy << ", " << pos_accuracy2 << ", " << pos_accuracy3 << std::endl;
      if (pos_accuracy <= 0)
        pos_accuracy = 1;
      // printf("receive covariance %lf \n", pos_accuracy);
      if (pos_accuracy < 14) {
//        if(pos_accuracy3 < 120)
          globalEstimator.inputGPS(t, latitude, longitude, altitude, pos_accuracy);
//        else
//          globalEstimator.inputGPS(t, latitude, longitude, vio_q.z(), pos_accuracy);
      }
      gpsQueue.pop();
      break;
  ...
  std::ofstream foutC("/home/xchu/udicreator_ws/src/output/vio_global.csv", ios::app);
...
int main(int argc, char **argv) {
  ...
  ROS_INFO("\033[1;32m----> Global Fusion Started.\033[0m");

  global_path = &globalEstimator.global_path;

  ros::Subscriber sub_GPS = n.subscribe("/fix", 100, GPS_callback);
  ros::Subscriber sub_vio = n.subscribe("/lio_sam_udi/mapping/odometry", 100, vio_callback);
